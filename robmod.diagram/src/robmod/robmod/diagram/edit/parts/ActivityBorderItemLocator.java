...
public class ActivityBorderItemLocator extends BorderItemLocator {
	...
	protected Point locateOnParent(Point suggestedLocation, int suggestedSide,
		...
		int westX = parentFigureX + parentFigureWidth - borderItemSize.width
		...
		if (getPreferredSideOfParent() == PositionConstants.WEST) {
			...
			if (suggestedLocation.y < bounds.getTopLeft().y + 20) {
				newY = northY + borderItemSize.height + 20;
	...
	protected Point getPreferredLocation(int side, IFigure borderItem) {
		...
		if (getPreferredSideOfParent() == PositionConstants.WEST) {
			int x = westX;
			int y = northY + borderItemSize.height + 20;
